...
Paste the GitHub webhook url you copied from the `inventory-pipeline` as the *Payload URL* and 
disable SSL verification by clicking on *Disable SSL verification* and then *I understand my 
webhooks may not be secure*. The reason for disabling SSL in this lab is that we are using 
