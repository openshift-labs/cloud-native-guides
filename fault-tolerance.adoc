...
it's not needed anymore, use the 'oc run' command:

[source,bash]
----
oc run web-load --generator=run-pod/v1 --image=jordi/ab -- ab -n 10000 -c 10 http://web:8080/
----

In the above, `--image` specified which container image should be deployed. OpenShift will first 
look in the internal image registry and then in defined upstream registries 
