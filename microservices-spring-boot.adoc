...
S2I uses the supported OpenJDK container image to build the final container image of Catalog service by uploading 
the WildFly Swam uber-jar from the `target` folder to the openshift platform. 
...
* Copy the route url and verify the Inventory service works using 'curl':

[source,bash]
----
$ curl http://INVENTORY-ROUTE-URL/api/catalog
